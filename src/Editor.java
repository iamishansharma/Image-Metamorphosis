...
public class Editor extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private boolean loadCorrespondences(String path, Image source_image, Image target_image,
            Vector<Line2D.Double> segments) {
        segments.clear();
        if (path != null) {
            Scanner in = null;
            try {
    ...
    private boolean saveCorrespondences(String path, Vector<Line2D.Double> segments) {
        ...
        for (int i = 0; i < segments.size(); i += 2) {
            ...
            Line2D.Double seg2 = (i + 1 >= segments.size() ? new Line2D.Double(0.0, 0.0, 0.0, 0.0)
                    : segments.elementAt(i + 1));

            writer.println(seg1.x1 + " " + seg1.y1 + " " + seg1.x2 + " " + seg1.y2 + " " + seg2.x1 + " " + seg2.y1 + " "
                    + seg2.x2 + " " + seg2.y2);
        }
    ...
    Color getPaletteColor(int i) {
        final Color PALETTE[] = { new Color(1.0f, 0.0f, 0.0f), new Color(0.0f, 1.0f, 0.0f), new Color(0.0f, 0.0f, 1.0f),
                new Color(1.0f, 0.0f, 1.0f), new Color(0.0f, 1.0f, 1.0f), new Color(1.0f, 1.0f, 0.0f),
                new Color(1.0f, 0.3f, 0.7f), new Color(1.0f, 0.7f, 0.3f), new Color(0.7f, 1.0f, 0.3f),
                new Color(0.3f, 1.0f, 0.7f), new Color(0.7f, 0.3f, 1.0f), new Color(0.3f, 0.7f, 1.0f),
                new Color(1.0f, 0.5f, 0.5f), new Color(0.5f, 1.0f, 0.5f), new Color(0.5f, 0.5f, 1.0f),
                new Color(1.0f, 0.5f, 1.0f), new Color(0.5f, 1.0f, 1.0f), new Color(1.0f, 1.0f, 0.5f),
                new Color(0.5f, 0.0f, 0.0f), new Color(0.0f, 0.5f, 0.0f), new Color(0.0f, 0.0f, 0.5f),
                new Color(0.5f, 0.0f, 0.5f), new Color(0.0f, 0.5f, 0.5f), new Color(0.5f, 0.5f, 0.0f) };
    ...
    private boolean parseArgs(String[] args) {
        ...
        for (int i = 0; i < args.length; ++i) {
            if (args[i].equals("-v"))
                ...
                case 0:
                    input_source_image_name = args[i];
                    break;
                case 1:
                    input_target_image_name = args[i];
                    break;
                case 2:
                    output_correspondence_name = args[i];
                    break;
                default:
                    System.err.println("Invalid program argument: " + args[i]);
                    return false;
                }
                current_positional++;
            }
